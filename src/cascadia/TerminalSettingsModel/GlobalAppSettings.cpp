...
void GlobalAppSettings::LayerJson(const Json::Value& json, const OriginTag origin)
{
    ...
    _logSettingValIfSet(json, DefaultProfileKey);
...
#undef GLOBAL_SETTINGS_LAYER_JSON
    ...
    _logSettingValIfSet(json, LegacyReloadEnvironmentVariablesKey);
}
...
void GlobalAppSettings::_logSettingSet(std::string_view setting)
{
    if (setting == "theme")
    {
        ...
        {
            _changeLog.insert(std::string_view{ fmt::format(FMT_COMPILE("{}.{}"), setting, "darkTheme") });
            _changeLog.insert(std::string_view{ fmt::format(FMT_COMPILE("{}.{}"), setting, "lightTheme") });
        }
    }
    else if (setting == "newTabMenu")
    {
        // TODO CARLOS: figure out how we want to track this
    }
    else
    {
        _changeLog.insert(setting);
...
void GlobalAppSettings::_logSettingValIfSet(const Json::Value& json, std::string_view setting)
{
...
void GlobalAppSettings::LogSettingChanges(std::vector<std::string_view>& changes) const
{
    for (const auto& setting : _changeLog)
    {
        changes.emplace_back(fmt::format(FMT_COMPILE("global.{}"), setting));
    }
}
